...
console.error(`Magic MCP is now the 21st MCP - proxying to ${endpoint} (shim v${VERSION}).`);
if (!apiKey) {
  console.error(
...
async function forward(line: string): Promise<void> {
  ...
  if (!text.trim()) {
    writeOut(rpcError(single?.id ?? null, -32000, `Empty response (HTTP ${res.status})`));
    return;
  }
  ...
  try {
    ...
    if (single && parsed && !Array.isArray(parsed) && parsed.id == null && single.id != null) {
      parsed.id = single.id;
    }
    writeOut(parsed);
  } catch {
