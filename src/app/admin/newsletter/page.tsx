...
export default function NewsletterIndexPage() {
  ...
  const handleSendCampaign = async (id: string) => {
    if (confirm('Are you sure you want to send this campaign to all active subscribers now?')) {
      ...
      try {
        const response = await fetch(`/api/admin/newsletter/campaigns/${id}/send`, { method: 'POST' });
        const res = await response.json();
        if (res.success) {
          toast.success(res.message, { id: toastId });
          fetchCampaigns();
  ...
  const filteredSubs = subscribers.filter((s) => s.email.toLowerCase().includes(search.toLowerCase()) || (s.name && s.name.toLowerCase().includes(search.toLowerCase())));
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <Tabs defaultValue="campaigns" className="w-full">
        <TabsList className="mb-4">
          ...
          <TabsTrigger value="subscribers"><Users className="h-4 w-4 mr-2" /> Subscribers</TabsTrigger>
        </TabsList>
            ...
            <CardHeader className="pb-4">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <div className="relative w-full sm:w-72">
                  ...
                  <Input placeholder="Search campaigns..." className="pl-9 bg-background/50" value={search} onChange={(e) => setSearch(e.target.value)} />
                </div>
              </div>
            </CardHeader>
            <CardContent>
                ...
                <Table>
                  ...
                  <TableBody>
                    {isLoadingCampaigns ? (
                      ...
                      <TableRow><TableCell colSpan={4} className="text-center py-8">No campaigns found.</TableCell></TableRow>
                    ) : (
                      filteredCampaigns.map((c) => (
                        <TableRow key={c.id}>
                          <TableCell className="font-medium">
                            <Link href={`/admin/newsletter/edit/${c.id}`} className="hover:text-indigo-500">
                              {c.subject}
                            </Link>
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell className="text-sm text-muted-foreground">
                            {c.status === 'SENT' ? `Sent: ${format(new Date(c.sentAt!), 'MMM d')}` : `Created: ${format(new Date(c.createdAt), 'MMM d')}`}
                          </TableCell>
                          <TableCell className="text-right">
                            <div className="flex items-center justify-end space-x-1">
                              {c.status !== 'SENT' && (
                                <Button variant="outline" size="sm" onClick={() => handleSendCampaign(c.id)} className="mr-2">
                                  <Send className="h-3 w-3 mr-1" /> Send
                                </Button>
                              )}
                              <Button asChild variant="ghost" size="icon-sm">
                                <Link href={`/admin/newsletter/edit/${c.id}`}><Edit className="h-4 w-4" /></Link>
                              </Button>
                              <Button variant="ghost" size="icon-sm" onClick={() => handleDeleteCampaign(c.id)} className="text-destructive">
          ...
          <Card className="shadow-sm border-border/50 bg-card/60 backdrop-blur-xl relative overflow-hidden">
            <CardHeader className="pb-4">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <div className="relative w-full sm:w-72">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                  <Input placeholder="Search subscribers..." className="pl-9 bg-background/50" value={search} onChange={(e) => setSearch(e.target.value)} />
                </div>
              </div>
            </CardHeader>
            <CardContent>
                ...
                <Table>
                  ...
                  <TableBody>
                    {isLoadingSubs ? (
                      ...
                      <TableRow><TableCell colSpan={4} className="text-center py-8">No subscribers found.</TableCell></TableRow>
                    ) : (
                      filteredSubs.map((s) => (
                        <TableRow key={s.id}>
