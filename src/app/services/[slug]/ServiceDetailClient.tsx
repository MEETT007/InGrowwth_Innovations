...
import { 
  SiPython, SiTensorflow, SiPytorch, SiHuggingface, SiDocker,
  SiReact, SiFlutter, SiSwift, SiKotlin, SiFirebase, SiNodedotjs,
  SiSolidity, SiEthereum, SiRust, SiIpfs,
  SiKalilinux, SiWireshark, SiSplunk,
  SiOdoo, SiPostgresql, SiJavascript
} from 'react-icons/si';
  ...
  return (
    <div className="flex flex-col min-h-screen relative overflow-hidden bg-background py-12">
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-6 pt-10 pb-20 w-full flex flex-col lg:flex-row items-center gap-16">
        ...
        >
          {/* Icon badge */}
          <div className={`p-4 w-fit rounded-2xl ${colorClass} shadow-lg ring-1 ring-indigo-500/20`}>
        ...
        >
          ...
          <div className="relative h-[450px] w-full flex items-center justify-center">
            ...
            >
              <IconComponent name={service.icon} className="w-40 h-40 text-indigo-500 drop-shadow-2xl" />
            </motion.div>
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-6 py-20 w-full border-t border-border/10">
        <AnimatedContainer direction="up" delay={0.1} className="max-w-4xl mx-auto relative">
           <div className="absolute -top-10 -left-6 text-9xl text-indigo-500/10 font-serif leading-none select-none">"</div>
           <h2 className="text-3xl font-bold mb-10 text-foreground text-center">About the Service</h2>
           <p className="text-xl sm:text-2xl text-foreground/80 leading-relaxed text-center font-medium relative z-10">
             {service.content}
           </p>
           <div className="absolute -bottom-16 -right-6 text-9xl text-indigo-500/10 font-serif leading-none select-none rotate-180">"</div>
        </AnimatedContainer>
      </section>
               ...
               <div className="relative z-10">
                 ...
                 <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6 justify-center items-center">
                   {service.techStack.map((tech, idx) => {
                     ...
                     else if (name.includes('openai') || name.includes('ai') || name.includes('tensor')) TechIcon = BrainCircuit;
                     else if (name.includes('web3') || name.includes('sql') || name.includes('oracle') || name.includes('mongo') || name.includes('database')) TechIcon = Database;
                     else if (name.includes('tableau') || name.includes('powerbi')) TechIcon = BarChartIcon;
                     else if (name.includes('azure') || name.includes('cloud')) TechIcon = Cloud;
                     else if (name.includes('security') || name.includes('crowdstrike')) TechIcon = Shield;
                     else if (name.includes('onestream') || name.includes('hyperion') || name.includes('erp') || name.includes('epm')) TechIcon = Briefcase;
